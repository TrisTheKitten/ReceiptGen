...
import { ReceiptData, DEFAULT_SCANNER_EFFECTS, DEFAULT_INVOICE_RECEIPT, DEFAULT_TAX_INVOICE_RECEIPT, randomizeScannerEffects, BackgroundStyle, TemplateStyle, LanguageMode } from '../types';
...
export const ReceiptForm: React.FC<Props> = ({
    ...
    const handleTemplateChange = (template: TemplateStyle) => {
        if (template === 'tax-invoice') {
            if (data.templateStyle === 'tax-invoice') {
                onChange({ ...data, templateStyle: 'tax-invoice' });
            } else {
                onChange({
                    ...DEFAULT_TAX_INVOICE_RECEIPT,
            ...
            onChange({
                ...
                language: data.language || 'th-en',
                sellerName: data.templateStyle === 'invoice' ? data.sellerName : DEFAULT_INVOICE_RECEIPT.sellerName,
                sellerAddress: data.templateStyle === 'invoice' ? data.sellerAddress : DEFAULT_INVOICE_RECEIPT.sellerAddress,
                sellerTaxId: data.templateStyle === 'invoice' ? data.sellerTaxId : DEFAULT_INVOICE_RECEIPT.sellerTaxId,
                buyerAddress: data.buyerAddress || DEFAULT_INVOICE_RECEIPT.buyerAddress,
                buyerTaxId: data.buyerTaxId || DEFAULT_INVOICE_RECEIPT.buyerTaxId,
            });
    ...
    const addItem = () => {
        const newItems = [...data.items, { id: Date.now().toString(), description: 'Item Name', quantity: 1, unitPrice: 10, discount: 0, total: 10 }];
        const { subtotal, grandTotal, vatAmount } = calculateTotals(newItems, data.savings || 0, data.vatRate);
        onChange({ ...data, items: newItems, subtotal, vatAmount, grandTotal });
    };
    ...
    const isInvoice = data.templateStyle === 'invoice';
    const isTaxInvoice = data.templateStyle === 'tax-invoice';
    const categoryId = isTaxInvoice ? 'tax-invoice' : isInvoice ? 'invoice' : isLazada ? 'lazada' : '7-eleven';
            ...
            <div className="px-5 pt-5 pb-3 bg-white sticky top-0 z-10 border-b border-gray-100">
                <div className="bg-gray-100 p-1 rounded-lg flex w-full relative">
                    <button
                        ...
                        className={`flex-1 py-2.5 text-xs font-bold rounded-md flex items-center justify-center gap-2 transition-all ${activeTab === 'editor'
                    ...
                    <button
                        ...
                        className={`flex-1 py-2.5 text-xs font-bold rounded-md flex items-center justify-center gap-2 transition-all ${activeTab === 'bulk'
                            ? 'bg-white text-black shadow-sm'
                            : 'text-gray-500 hover:text-gray-800'
                            }`}
                    >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
                {activeTab === 'editor' ? (
                    <div className="space-y-4 pb-24">
                        ...
                        <div className="bg-white p-1 rounded-xl border border-gray-200">
                            <div className="grid grid-cols-4 gap-1">
                                <button
                                    ...
                                    className={`py-3 rounded-lg text-xs font-bold transition-all border ${data.templateStyle === 'thermal' ? 'bg-black text-white border-black' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
                                >
                                    7-Eleven
                                </button>
                                <button
                                ...
                                <button
                                    ...
                                    className={`py-3 rounded-lg text-xs font-bold transition-all border ${isInvoice ? 'bg-black text-white border-black' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
                                >
                                    Invoice
                                </button>
                                <button
                                    onClick={() => handleTemplateChange('tax-invoice')}
                                    className={`py-3 rounded-lg text-xs font-bold transition-all border ${isTaxInvoice ? 'bg-black text-white border-black' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
                                >
                                    Tax Inv.
                                </button>
                            </div>
                        ...
                        {(isInvoice || isTaxInvoice) && (
                            <div className="bg-white p-1 rounded-xl border border-gray-200">
                                <div className="grid grid-cols-2 gap-1">
                                    <button
                                        onClick={() => onChange({ ...data, language: 'th' as LanguageMode })}
                                        className={`py-2.5 rounded-lg text-xs font-bold transition-all border ${data.language === 'th' ? 'bg-black text-white border-black' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
                                    >
                                        ภาษาไทย
                                    </button>
                                    <button
                                        onClick={() => onChange({ ...data, language: 'th-en' as LanguageMode })}
                                        className={`py-2.5 rounded-lg text-xs font-bold transition-all border ${data.language === 'th-en' ? 'bg-black text-white border-black' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
                                    >
                                        {isTaxInvoice ? 'English' : 'ไทย / English'}
                                    </button>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-gray-900 tracking-tight">
                                        {isTaxInvoice ? 'Tax Invoice Generator' : isInvoice ? 'Invoice Data Generator' : isLazada ? 'Lazada Data Generator' : '7-Eleven Data Generator'}
                                    </h3>
                                    <p className="text-[11px] text-gray-400 mt-1 uppercase tracking-wider font-bold">
                                        {isTaxInvoice ? 'Company & product details' : isInvoice ? 'Company & service details' : isLazada ? 'Logistics & tracking info' : 'Items & member data'}
                                    </p>
                                </div>
                        ...
                        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                            <div className="flex items-center justify-between p-4 border-b border-gray-100">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => {
                                            const randomized = randomizeScannerEffects(data.scannerEffects.extremeMode);
                                            onChange({
                                                ...data,
                                                scannerEffects: {
                                                    ...data.scannerEffects,
                            ...
                            {data.scannerEffects.enabled && (
                                <div className="p-4 space-y-4">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Extreme Mode</span>
                                            <span className="text-[9px] font-bold text-orange-500 uppercase tracking-wider">(Very Hard to Read)</span>
                                        </div>
                                        <button
                                            onClick={() => {
                                                const newExtremeMode = !data.scannerEffects.extremeMode;
                                                if (newExtremeMode) {
                                                    const extremeEffects = randomizeScannerEffects(true);
                                                    onChange({
                                                        ...data,
                                                        scannerEffects: {
                                                            ...data.scannerEffects,
                                                    ...
                                                    onChange({
                                                        ...data,
                                                        scannerEffects: { ...data.scannerEffects, extremeMode: false }
                                                    });
                                                }
                                            }}
                                            className={`h-7 px-3 rounded-md text-[10px] font-bold transition-all border ${data.scannerEffects.extremeMode ? 'bg-orange-600 text-white border-orange-600' : 'bg-white text-gray-400 border-gray-200'}`}
                                            aria-pressed={data.scannerEffects.extremeMode}
                                        >
                                            {data.scannerEffects.extremeMode ? 'ON' : 'OFF'}
                                        </button>
                                    </div>
                                        ...
                                        <div className="grid grid-cols-5 gap-1.5">
                                            {([
                                                ...
                                                <button
                                                    ...
                                                    className={`flex flex-col items-center gap-1 p-2 rounded-lg border-2 transition-all ${data.scannerEffects.background === bg.id
                                                        ? 'border-black'
                                    ...
                                    <ScannerSlider
                                        label="Zoom"
                                        value={data.scannerEffects.zoom}
                                        min={0.85} max={1.2} step={0.01}
                                        unit="x"
                                        onChange={(v) => onChange({ ...data, scannerEffects: { ...data.scannerEffects, zoom: v } })}
                                    />
                        ...
                        <Section title="Customer Info" isOpen={openSections['buyer']} onToggle={() => toggleSection('buyer')} isSummary={data.buyerName}>
                            ...
                            {(isLazada || isInvoice || isTaxInvoice) && (
                            ...
                            {(isInvoice || isTaxInvoice) && (
                                <InputGroup label="Customer Tax ID">
                                    <input type="text" name="buyerTaxId" value={data.buyerTaxId || ''} onChange={handleInputChange} className={inputBaseClass} />
                                </InputGroup>
                            )}
