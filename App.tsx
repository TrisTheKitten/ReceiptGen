...
export default function App() {
  ...
  const handleApiKeyChange = (key: string, save: boolean) => {
      setApiKey(key);
      setShouldSaveKey(save);
      if (save) {
  ...
  const handleGenerateAI = async (category: string) => {
    ...
    try {
      ...
      const generatedData = await generateReceiptData(category, data, keyToUse);
      if (generatedData) {
        setData(prev => ({
          ...prev,
          ...generatedData,
  ...
  const handleBulkGenerate = async (count: number, mode: 'random' | 'current', currentCategory?: string) => {
    ...
    try {
        ...
        const batchResults = await generateReceiptBatch(currentCategory || "7-eleven", count, keyToUse);
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden select-none">
      {notification && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:right-6 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 z-[60] animate-in slide-in-from-top-4 duration-300 border ${notification.type === 'error' ? 'bg-red-50 text-red-900 border-red-100' : 'bg-emerald-800 text-white border-emerald-700'}`}>
      ...
      <aside 
        ...
        style={{ width: window.innerWidth >= 768 ? (isSidebarCollapsed ? 0 : sidebarWidth) : '85%', maxWidth: '400px', opacity: (window.innerWidth >= 768 && isSidebarCollapsed) ? 0 : 1 }}
      >
          <div className="h-16 px-6 border-b border-gray-100 flex items-center justify-between bg-white whitespace-nowrap overflow-hidden shrink-0">
              ...
              <button className="md:hidden p-2 text-gray-400 hover:text-gray-600" onClick={() => setIsMobileMenuOpen(false)}>
